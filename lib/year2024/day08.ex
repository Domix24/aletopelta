defmodule Aletopelta.Year2024.Day08 do
  ...
  defmodule Part1 do
    ...
    def execute(input) do
      ...
      |> Enum.uniq()
      |> Enum.count(
        &(elem(&1, 0) > -1 and elem(&1, 0) < map_width and elem(&1, 1) > -1 and
            elem(&1, 1) < map_height)
      )
    end
  ...
  defmodule Part2 do
    ...
    defp map_distance({x1, y1}, {x2, y2}, {max_x, max_y}) do
      {{x1, y1}, {x2, y2},
       fill_antinodes({x1, y1}, {x2, y2}, {x1 - x2, y1 - y2, x2 - x1, y2 - y1}, {max_x, max_y})}
    end
    ...
    defp fill_antinodes(
           {x1, y1},
           {x2, y2},
           {delta_x_left, delta_y_left, delta_x_right, delta_y_right},
           {max_x, max_y}
         ) do
    ...
    defp fill_antinodes({x, y}, {delta_x, delta_y}, {max_x, max_y})
         when x + delta_x > -1 and y + delta_y > -1 and x + delta_x < max_x and
                y + delta_y < max_y do
      [
        {x + delta_x, y + delta_y}
        | fill_antinodes({x + delta_x, y + delta_y}, {delta_x, delta_y}, {max_x, max_y})
      ]
    end
