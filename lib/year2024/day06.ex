defmodule Aletopelta.Year2024.Day06 do
  ...
  defmodule Common do
    ...
    def parse_input([first_line | rest], row_index) do
      new_first_line = for {char, index} <- String.graphemes(first_line) |> Enum.with_index() do
  ...
  defmodule Part1 do
    ...
    defp move_guard(map, {guard_x, guard_y, guard_direction}) do
      ...
      {next_guard_x, next_guard_y} = Common.calculate_next_position(guard_x, guard_y, guard_direction)
      ...
      case get_symbol(map, next_guard_x, next_guard_y, map_width, map_height, loop_wall_position) do
        ...
        "#" ->
          if Map.has_key?(walls, {next_guard_x, next_guard_y, guard_direction}) do
            ...
            handle_rotation(map, {guard_x, guard_y, guard_direction},
                            initial_position, map_width, map_height, new_walls, blocking_walls, loop_wall_position)
          end
        _ ->
          if loop_wall_position == nil and {next_guard_x, next_guard_y} != initial_position
            and not Map.has_key?(blocking_walls, {next_guard_x, next_guard_y}) do
            new_blocking_walls = Map.put(blocking_walls, {next_guard_x, next_guard_y}, true)

            move_with_loop = move(map, {guard_x, guard_y, guard_direction},
                                  initial_position, map_width, map_height,
                                  walls, new_blocking_walls, {next_guard_x, next_guard_y})
            move_without_loop = move(map, {next_guard_x, next_guard_y, guard_direction},
                                     initial_position, map_width, map_height, walls, new_blocking_walls, nil)

            move_with_loop + move_without_loop
          else
            map
            |> move({next_guard_x, next_guard_y, guard_direction},
                     initial_position, map_width, map_height, walls, blocking_walls, loop_wall_position)
          end
      end
    end
