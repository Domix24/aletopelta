defmodule Aletopelta.Year2024.Day10 do
  @moduledoc """
  Day 10 of Year 2024
  """
  defmodule Common do
    ...
    def build_path({_x, _y, n} = starting_point, map, map_info) do
      ...
      |> Enum.map(fn {_delta, next_point} ->
        rest_build_path =
          build_path(next_point, map, map_info)
    ...
    defp get_next_point({x, y, _n}, {dx, dy}, map, {max_x, max_y})
         when x + dx > -1 and y + dy > -1 and x + dx < max_x and y + dy < max_y do
    ...
    def highest?({_, _, 9}), do: true
    def highest?(_), do: false
  end
