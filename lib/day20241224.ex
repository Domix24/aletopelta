defmodule Aletopelta.Day20241224 do
  ...
  defmodule Part2 do
    ...
    defp is_wrong({:AND, "x00", "y00", _}, _), do: :good
    defp is_wrong({:AND, _, _, result}, gates) do
    ...
    defp is_wrong({:OR, _, _, "z45"}, _), do: :good
    defp is_wrong({:OR, _, _, "z" <> _ = result}, _), do: {:wrong, result}
    defp is_wrong({:OR, _, _, result}, gates) do
      case Enum.find_value(gates, fn
        {:OR, ^result, _, _} -> true
        {:OR, _, ^result, _} -> true
        _ -> false
    ...
    defp is_wrong({:XOR, "x00", "y00", "z00"}, _), do: :good
    defp is_wrong({:XOR, <<a, _, _>>, <<b, _, _>>, result}, gates) when a == ?x when b == ?x do
      case Enum.find_value(gates, fn
        {:XOR, ^result, _, _} -> true
        {:XOR, _, ^result, _} -> true
        _ -> false
    ...
    defp is_wrong({:XOR, _, _, "z" <> _}, _), do: :good
    defp is_wrong({:XOR, _, _, result}, _), do: {:wrong, result}
  end
end
