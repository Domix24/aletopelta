defmodule Aletopelta.Day20241215 do
  ...
  defmodule Part2 do
    ...
    defp build_object_entries(map, {x, y} = position, {dx, dy} = delta, direction, primary_type, secondary_type) do
      ...
      case delta do
        {0, _} -> entries ++ build_object_list(map, new_primary_position, delta) ++ build_object_list(map, new_adjacent_position, delta)
        {_, 0} -> entries ++ build_object_list(map, new_primary_position, delta)
      end
    end
