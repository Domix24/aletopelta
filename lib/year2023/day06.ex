defmodule Aletopelta.Year2023.Day06 do
  ...
  defmodule Common do
    ...
    def get_zeros [%{time: time, distance: distance} | others] do
      ...
      positive = (Integer.parse Float.to_string (String.to_float("-#{time}.0") + quadratic) / -2.0)
      |> then(fn {number, _} -> number + 1 end)
      negative = (Integer.parse Float.to_string (String.to_float("-#{time}.0") - quadratic) / -2.0)
      |> case do
        {number, ".0"} -> number - 1
        {number, _} -> number
      end
