defmodule Aletopelta.Year2023.Day16 do
  ...
  defmodule Common do
    ...
    def parse_input(input) do
      {result, _} =
        Enum.reduce(input, {%{}, 0}, fn list, {acc, row_index} ->
          {acc, _} =
      ...
      cond do
        ...
        true ->
          ...
          |> case do
            :empty ->
              visited =
                Map.update(visited, position, [{direction, :empty}], &[{direction, :empty} | &1])

              traverse(map, position, direction, visited)

            {mirror, indications} ->
              visited =
                Map.update(visited, position, [{direction, mirror}], &[{direction, mirror} | &1])
  ...
  defmodule Part2 do
    ...
    def do_loop(input) do
      ...
      all_positions =
        for y <- 0..max_y, reduce: all_positions do
