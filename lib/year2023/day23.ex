defmodule Aletopelta.Year2023.Day23 do
  ...
  defmodule Common do
    ...
    def parse_input(input) do
      {map, _} =
        Enum.reduce(input, {Map.new(), 0}, fn line, {map, row} ->
          {map, _} =
    ...
    def find_endpoints(grid) do
      {{min_y, min_tiles}, {max_y, max_tiles}} =
        Enum.group_by(
          grid,
          fn
            {{_, y}, _} -> y
          end,
          fn {{x, _}, type} -> {x, type} end
        )
        |> Enum.min_max_by(fn {y, _} -> y end)

      start_pos =
        Enum.find_value(min_tiles, fn
          {x, :empty} -> {x, min_y}
          _ -> false
        end)

      end_pos =
        Enum.find_value(max_tiles, fn
          {x, :empty} -> {x, max_y}
          _ -> false
        end)
  ...
  defmodule Part2 do
    ...
    defp build_graph(grid, start_pos, end_pos) do
      walkable =
        Enum.filter(grid, &(elem(&1, 1) == :empty))
        |> Enum.map(&elem(&1, 0))
        |> MapSet.new()
