defmodule Aletopelta.Year2023.Day13 do
  ...
  defmodule Common do
    ...
    def parse_input(input) do
      Enum.chunk_while(input, [], fn
        "", acc -> {:cont, Enum.reverse(acc) |> create_grid, []}
        line, acc -> {:cont, [line | acc]}
      end, fn acc -> {:cont, Enum.reverse(acc) |> create_grid, []} end)
    end
  ...
  defmodule Part2 do
    ...
    def find_reflexion input, get_key, get_value, is_same? do
      ...
      {state, _, choices} = Enum.reduce_while(sums, {nil, Map.to_list(sums), MapSet.new}, fn
    ...
    defp find_onedifference choices, acc, child_key, key, sum, child_sum do
      cond do
        rem(abs(child_key - key), 2) == 0 -> {:cont, acc}
        Bitwise.band(Bitwise.bxor(sum, child_sum), Bitwise.bxor(sum, child_sum) - 1) == 0 -> {:cont, {:found, MapSet.put(choices, {key, child_key, [sum, child_sum]})}}
        true -> {:cont, acc}
      end
    end
