defmodule Aletopelta.Year2023.Day14 do
  ...
  defmodule Common do
    ...
    def prepare_input input do
      {rocks, blocking_rocks, empty_spaces, [min_x, max_x], [min_y, max_y]} = Enum.reduce(input, {[], MapSet.new, MapSet.new, [nil, nil], [nil, nil]}, fn {{x, y} = k, value}, {rocks1, rocks2, spaces, [bound0, bound2], [bound1, bound3]} ->
      ...
      {rocks, blocking_rocks, empty_spaces, {MapSet.new([min_x, max_x]), MapSet.new([min_y, max_y])}, {bitmap, calculate_bitmap}}
    end
    ...
    def move({[], blocking_rocks, empty_spaces, invalid_spaces, {_, total_rocks}, bitmap_info}, _), do: {total_rocks, blocking_rocks, empty_spaces, invalid_spaces, bitmap_info}
    def move({[rock | rocks], blocking_rocks, empty_spaces, invalid_spaces, {visited_rocks, total_rocks} = visited_spaces, {bitmap, calculate_bitmap} = bitmap_info}, direction) do
      {dx, dy} = delta = get_delta direction

      if MapSet.member? visited_rocks, rock do
        move {rocks, blocking_rocks, empty_spaces, invalid_spaces, visited_spaces, bitmap_info}, direction
        ...
        {visited_rocks, slide_rocks, limit, slide_count} =
          get_slide(rock,
                    delta,
                    {blocking_rocks, empty_spaces, invalid_spaces, visited_rocks},
                    slide_rocks, slide_count)

        visited_rocks = MapSet.put visited_rocks, rock

        {_, empty_spaces, _, total_rocks, bitmap} =
          for _ <- 1..slide_count,
          reduce: {limit, empty_spaces, slide_rocks, total_rocks, bitmap}
          do
        ...
        move {rocks, blocking_rocks,
              empty_spaces, invalid_spaces,
              {visited_rocks, total_rocks}, {bitmap, calculate_bitmap}}, direction
      end
    end

    defp get_slide {x, y}, {dx, dy}, {blocking_rocks, empty_spaces, {invalid_horizontal, invalid_vertical} = invalid_spaces, visited_rocks}, result, count do
      {nx, ny} = {x + dx, y + dy}
      ...
      cond do
        ...
        MapSet.member? empty_spaces, {nx, ny} ->
          get_slide {nx, ny}, {dx, dy}, {blocking_rocks, empty_spaces, invalid_spaces, visited_rocks}, result, count

        true ->
          visited_rocks = MapSet.put visited_rocks, {nx, ny}
          result = MapSet.put result, {nx, ny}

          get_slide {nx, ny}, {dx, dy},
                    {blocking_rocks, empty_spaces, invalid_spaces, visited_rocks}, result, (count + 1)
      end
    end
  end
    ...
    defp do_cycle(input, index, max, visited) do
      ...
      case Map.fetch visited, bitmap do
        :error ->
          ...
          Enum.reduce([:north, :west, :south, :east], input, fn direction, {rocks, blocking_rocks, empty_spaces, invalid_spaces, bitmap} ->
            Common.move {rocks, blocking_rocks, empty_spaces, invalid_spaces, {MapSet.new, []}, bitmap}, direction
          end) |> do_cycle(index + 1, max, visited)
        {:ok, cycle} ->
          size = index - cycle
          offset = max - index
