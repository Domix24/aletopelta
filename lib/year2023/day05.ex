defmodule Aletopelta.Year2023.Day05 do
  ...
  defmodule Common do
    ...
    def parse_input(["seeds:" <> numbers | others], nil, nil), do: [{:seeds, String.split(numbers) |> Enum.map(&String.to_integer/1)} | parse_input(others, nil, nil)]
    def parse_input([first | others], from, to) do
      ...
      if length(match) == 1 do
        ...
        [{{from, to}, String.split(first) |> Enum.map(&String.to_integer/1)} | parse_input(others, from, to)]
      end
    end
  end
  ...
  defmodule Part1 do
    ...
    defp find_location(source_value, almanac, source_key) do
     ...
     if result == :error do
       ...
       {source_key, source_value} = elem(result, 1)
    ...
    defp find_combinaison([[dest_range, source_range, length] | others], dest_key, source_value) do
      cond do
        source_range + length - 1 < source_value -> find_combinaison(others, dest_key, source_value)
        source_range > source_value -> find_combinaison(others, dest_key, source_value)
        true -> {dest_key, dest_range + source_value - source_range}
      end
    end
  end
  ...
  defmodule Part2 do
    ...
    defp find_combinaison([], dest_key, source_value), do: {dest_key, [source_value]}
    defp find_combinaison([[dest_range, source_range, length] | others] = almanac, dest_key, [source_start, source_length] = source_value) do
      ...
      if Range.disjoint?(almanac_range, input_range) do
        ...
        cond do
          input_range.first < almanac_range.first ->
            ...
            {_  , list_left}  = find_combinaison almanac, dest_key, [left_range.first, left_range.last - left_range.first + 1]
            {key, list_input} = find_combinaison almanac, dest_key, [input_range.first, input_range.last - input_range.first + 1]
            {key, list_left ++ list_input}

          input_range.last < almanac_range.last + 1 ->
            left_range = Range.new(destination_range.first + input_range.first - almanac_range.first, destination_range.first + input_range.last - almanac_range.first)
            {dest_key, [[left_range.first, left_range.last - left_range.first + 1]]}

          true ->
            left_range = Range.new(destination_range.first + input_range.first - almanac_range.first, destination_range.first + almanac_range.last - almanac_range.first)
            input_range = Range.new(almanac_range.last + 1, input_range.last)
            {key, list} = find_combinaison(almanac, dest_key, [input_range.first, input_range.last - input_range.first + 1])
