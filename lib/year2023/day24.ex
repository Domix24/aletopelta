defmodule Aletopelta.Year2023.Day24 do
  ...
  defmodule Part1 do
    ...
    defp chunk(stones) do
      Enum.reduce(stones, {stones, []}, fn stone1, {acc, res} ->
        ...
        res =
          Enum.reduce(stones, res, fn stone2, res ->
            [{stone1, stone2} | res]
          end)
  ...
  defmodule Part2 do
    ...
    def execute(input) do
      ...
      [rx: x, ry: y, rz: z, vx: _, vy: _, vz: _] =
        (create_equations(stone1, stone2) ++ create_equations(stone2, stone3))
        |> do_gauss
    ...
    defp substitute([equation | equations], atom, result) do
      sum =
        Enum.sum_by(equation, fn {key, value} ->
          cond do
            key == atom -> 0
            key == :c -> 0
            true -> value
